  ...
  options.forEach(function(option) {
    ...
    if (arrayOfOptionsSupported.indexOf(option.value) !== -1) {
      ...
      if (localStorageItem && arrayOfOptionsSupported.indexOf(localStorageItem) != -1) {
        if (option.value != localStorageItem) {
          return;
        }
...
function uploadToPHPServer(fileName, recordRTC, callback) {
  ...
  //var root = 'https://webrtcweb.com/RecordRTC/';
  //var root = '/';
  var root = 'RecordRTC-to-PHP/';

  makeXMLHttpRequest(root + 'save.php', formData, function(progress) {
    if (progress !== 'upload-ended') {
      callback(progress);
      return;
    }

    var initialURL = root + 'uploads/';
...
window.addEventListener('message', function(message) {
  if (message.origin != 'https://www.webrtc-experiment.com') {
    return;
  }
